...
class AspenBatchResult:
    ...
    def succeeded(self) -> bool:
        return (
            ...
            and history_diagnostics_are_clean(self.history_diagnostics)
        )
...
def parse_aspen_history(
    ...
    *,
    ...
    for index, line in enumerate(lines):
        ...
        if re.match(r"\s*(TERMINAL\s+ERRORS|SEVERE\s+ERRORS|ERRORS|WARNINGS)\s+\d", upper_line):
            continue
        if "TERMINAL ERROR" in upper_line:
            severity = "terminal"
        elif "SEVERE ERROR" in upper_line:
            severity = "severe"
    ...
    return {
        ...
        "blocking_error_count": total_errors,
        "messages": messages,
        "message_count": len(messages),
    }


def history_diagnostics_are_clean(history_diagnostics: dict[str, Any]) -> bool:
    """Return True only when Aspen history proves a clean batch run."""
    if not isinstance(history_diagnostics, dict):
        return False

    if str(history_diagnostics.get("status", "")).strip().lower() != "converged":
        return False

    if bool(history_diagnostics.get("input_translation_failed")):
        return False
    ...
    for message in history_diagnostics.get("messages", []):
        if not isinstance(message, dict):
            continue
        if str(message.get("severity", "")).strip().lower() in {"terminal", "severe", "error"}:
            return False
